...
resource "aws_inspector_assessment_target" "target" {
  ...
  name = var.aws_inspector_assessment_target_name
}

resource "aws_inspector_assessment_template" "assessment" {
  count = local.create ? 1 : 0

  name               = var.aws_inspector_assessment_template_name
...
#-----------------------------------------------------------------------------------------------------------------------
locals {
  create          = var.enable_aws_inspector && length(var.enabled_rules) > 0
